...
<div id="contenu">
    ...
    <p>Voici les dates où vous avez confirmé votre disponibilité :</p>
    ...
    <form name="frmConsultFrais" method="POST" action="index.php?choixTraitement=intervenant&action=valideModif">
        <hr>
        <h3>Ajouter des dates de disponibilité</h3>
        <p>Toute confirmation de votre présence ne peut être annulée, en cas d'empêchement de dernière minute, merci de
            contacter l'association.</p>
        <div class="alert alert-info"><strong>Pour rappel, voici les dates des vacances scolaires :</strong><br>
            <?php foreach ($lesVacances as $uneVacance) {
                echo nl2br($uneVacance['NOM']);
            } ?></div>
        <label for="date">Cochez vos disponibilités :<br></label><br><br>
        <?php
