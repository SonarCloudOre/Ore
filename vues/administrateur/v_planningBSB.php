<div id="contenu">
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>
                    Rendez-vous
                    <div class="page-title-subheading">Planning BSB</div>
                </div>
            </div>
    ...
    function mois_suivant($m, $a)
    {
        ...
        return "<a style='color:white' href=index.php?choixTraitement=administrateur&action=planningBSB&m=$m&a=$a> &raquo; </a>";
    }
    ...
    function mois_precedent($m, $mois, $a)
    {
        ...
        return "<a style='color:white' href=index.php?choixTraitement=administrateur&action=planningBSB&m=$m&a=$a> &laquo; </a>";
    }
  ...
  <table id=calendrier border=1 bordercolor=\"#FFFFFF\"><tr><td class=\"fleches\">"
            ...
            foreach ($lesRendezvous as $unRdv) {
                ...
                if (substr($unRdv['DATE_RDV'], 0, 10) == $a . '-' . $moisRdv . '-' . $jourRdv) {
                    ...
                    echo '<div class="unRdv">
						...
						<a style="color:white" href="index.php?choixTraitement=administrateur&action=LesEleves&unEleve=' . $unRdv['ID_ELEVE'] . '"><b>' . $unRdv['NOM'] . ' ' . $unRdv['PRENOM'] . '</b></a>
						<br>en ';
                    ...
                    echo '<br>pendant ' . $unRdv['DUREE'] . ' heure(s)
                        <br>avec <a style="color:white" href="index.php?choixTraitement=administrateur&action=LesIntervenants&unIntervenant=' . $unRdv['ID_INTERVENANT'] . '"><b>' . $unIntervenant['NOM'] . ' ' . $unIntervenant['PRENOM'] . '</b></a><br>
						<br><small><i>' . stripslashes($unRdv['COMMENTAIRE']) . '</i></small>';
                    if ($admin == 2 or $semaineAujourdhui == $semaineEnCours or strtotime($unRdv['DATE_RDV']) > time()) {
                        echo '<br><a href="javascript:if(confirm(\'Voulez-vous vraiment supprimer ce rendez-vous ?\')) { document.location.href=\'index.php?choixTraitement=administrateur&action=supprimerRdv&page=planningBSB&num=' . $unRdv['ID_RDV'] . '\'; };" class="btn btn-danger"><span class="glyphicon glyphicon-trash" style="width:10px"></span></a>';
                    }
                    echo '</div>';
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="main-card mb-3 card">
                <div class="card-body">
                    <h3 id="form_ajout" class="card-title">Ajouter un rendez-vous</h3><br>
                    <form name="frmConsultFrais" method="POST" enctype="multipart/form-data"
                          action="index.php?choixTraitement=administrateur&action=ajoutRdvBsb">
                        <div class="form-group">

                            <label for="num">Enfant </label>
                            <select class="form-control" name="num" style="width:200px" required class="form-control"
                            ...
                            <select class="form-control" name="matiere" style="width:200px" required
                                    class="form-control" data-live-search="true">
                                <option disabled selected>Choisir</option>
                                <?php
                                foreach ($lesMatieres as $uneMatiere) {
                                    echo '<option value="' . $uneMatiere['ID'] . '">' . $uneMatiere['NOM'] . '</option>';
                            ...
                            <label for="duree_inscription">Durée du rendez-vous (en heures)</label>
                            <input class="form-control" name="duree" autofocus="" value="1" style="width:200px"
                                   type="text" required><i>(mettre 1.5 pour 1H30, etc...)</i><br><br>

                            <label for="commentaire">Thème</label>
